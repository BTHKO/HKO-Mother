    ...
    try:
        ...
        if file_input.startswith("local:"):
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                raw_text = f.read()
            print(f"... Fetched local file: {file_name}")
        else:
            ...
            raw_text = drive.download_file(file_id, file_name)
        # --- End of Update ---
        ...
        output_filename = f"{datetime.now().strftime('%Y-%m-%d')}-{file_name}.html"
        output_path = os.path.join(OUTPUT_DIR, output_filename)

        # Convert to simple HTML
        html_content = f"<html><head><title>{file_name}</title></head><body>"
        ...
        # 5. Update Manifest
        update_manifest(file_name, output_filename)
        print(f"---== WORKFLOW COMPLETE FOR: {file_name} ==---")
